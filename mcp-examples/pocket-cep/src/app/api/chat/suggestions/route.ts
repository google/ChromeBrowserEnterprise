  ...
  try {
    ...
    const result = await generateObject({
      ...
      system:
        ...
        "Based on the preceding dialogue, formulate exactly 3 brief, actionable follow-up requests or prompts (under 8 words each) that the administrator might submit next. Try to make suggestions natural extensions of the main text that maintain conversational flow and require little thought to understand (i.e. do not propose something subtly different than what the assistant just offered or discussed). Use concise imperative statements (e.g. 'Run environment health check'). Do not put question marks at the end of imperative statements.\n\n" +
        "Alignment Guidance:\n" +
    ...
    const cleanQuestions = result.object.questions.map((q) => {
      const trimmed = q.trim();
      if (/^(Run|Show|Check|List|Audit|Investigate|Inspect|Review|Get|Verify)\b.*\?$/i.test(trimmed)) {
